...
type UserEsProvider struct {
	index   string
	mapping string
	client  *elastic.Client
}

func NewUserEsProvider(client *elastic.Client) *UserEsProvider {
	userEs := &UserEsProvider{
		client:  client,
...
func (es *UserEsProvider) init() {
	ctx := context.Background()

	exist, err := es.client.IndexExists(es.index).Do(ctx)
	if err != nil {
		log.Printf("userEs init exist failed err is %s\n", err)
		return
	}
	...
	if !exist {
		...
		if err != nil {
			log.Printf("userEs init failed err is %s\n", err)
			return
		}
	}
}

func (es *UserEsProvider) BatchAdd(ctx context.Context, user []*model.UserEs) error {
	var err error
	for i := 0; i < esRetryLimit; i++ {
		if err = es.batchAdd(ctx, user); err != nil {
			log.Println("batch add failed ", err)
			continue
		}
		return err
	}
...
func (es *UserEsProvider) BatchUpdate(ctx context.Context, user []*model.UserEs) error {
	var err error
	for i := 0; i < esRetryLimit; i++ {
...
func (es *UserEsProvider) BatchDel(ctx context.Context, user []*model.UserEs) error {
	var err error
	for i := 0; i < esRetryLimit; i++ {
...
// MGet 根据id 批量获取
func (es *UserEsProvider) MGet(ctx context.Context, IDS []uint64) ([]*model.UserEs, error) {
...
func (es *UserEsProvider) Search(ctx context.Context, filter *model.EsSearch) ([]*model.UserEs, error) {
