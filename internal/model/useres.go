...
type UserEs struct {
	...
	Age        uint64 `json:"age" mapstructure:"age"`
	Ancestral  string `json:"ancestral" mapstructure:"Ancestral"`
	Identity   string `json:"identity" mapstructure:"identity"`
	UpdateTime uint64 `json:"update_time" mapstructure:"update_time"`
	CreateTime uint64 `json:"create_time" mapstructure:"create_time"`
}
...
func (r *SearchRequest) ToFilter() *EsSearch {
	var search EsSearch
	if len(r.Nickname) != 0 {
		search.ShouldQuery = append(search.ShouldQuery,
			elastic.NewMatchQuery("nickname", r.Nickname) /*模糊查询，匹配查询*/)
	}
	if len(r.Phone) != 0 {
		search.ShouldQuery = append(search.ShouldQuery,
			elastic.NewTermsQuery("phone", r.Phone) /*采用精确查询*/)
	}
